...
void f_cwr(const int fd, void* fbf, const int fsz, const int nobj)
{
    ...
    long int bytestowrite = (long)fsz * nobj;

    if (write(fd, (const void*)fbf, (size_t)bytestowrite) != bytestowrite)
    {
...
{
    /* will write nobj * fsz elements from fbf */
    long int bytestoread = (long)fsz * nobj;

    if (read(fd, (void*)fbf, (size_t)bytestoread) != bytestoread)
    {
